...
struct TokenInstanceAction {
    ...
    func activeExcludingSelection(selectedTokenHolders: [TokenHolder]) -> TokenScriptSelection? {
        switch type {
        case .erc20Send, .erc20Receive:
            return nil
        case .nftRedeem, .nftSell, .nonFungibleTransfer:
        ...
        case .tokenScript(_, _, _, _, _, let selection):
            ...
            guard let (attribute: attribute, value: value, operator: op) = TokenScriptFilterParser(expression: selection.filter).parse() else { return nil }
            //TODO handle multiple TokenHolder. We only do single-selections now
            let tokenHolder = selectedTokenHolders[0]
            let filterExpressionIsTrue = op.isTrueFor(tokenHolder: tokenHolder, attribute: attribute, value: value)
